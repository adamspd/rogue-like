...
public class Event  {
    ...
    private static int entier_si_pas_espace = 0;
    ...
    private static boolean freeSpace(Grille grille, Salle salle_aleatoire, int posX) {
        ...
        {return false;}
        for (int j = -1-stairs_length + entier_si_pas_espace; j < 0; j++) {
            for (int i = -1 + entier_si_pas_espace; i < 2 - entier_si_pas_espace; i++) { //Un espace à gauche et à droite
                try {
    ...
    public static void genererateStairs(Grille grille) {    //Escalier présent, mais caché et inaccessible
        ...
        //Si on n'en trouve pas, la longueur de l'escalier est réduite au minimum.
        catch (Exception e){
            entier_si_pas_espace = 1;
            stairs_length = 1;
